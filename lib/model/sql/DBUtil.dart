...
import 'package:bobi_pay_out/model/sql/Constants.dart';
import 'package:bobi_pay_out/model/sql/tablesInit.dart';
import 'package:bobi_pay_out/utils/debug_info.dart';

final DBUtil dbMgr = DBUtil();
final TablesInit tablesInit = TablesInit();

///sqflite数据库操作工具类
class DBUtil {
  Future<List<String>> initDB() async {
    return await tablesInit.initDB();
  }
  ...
  ///sql原生查找列表
  Future<List<Map<String, dynamic>>> queryList(String sql) async {
  ...
  open({
    ...
    if (_dbCount > 0 && db.isOpen) {
      _dbCount++;
      return;
    }
    String databasePath = await getDatabasesPath();
    String path = join(databasePath, Constants.DB_NAME);
    mypdebug('数据库存储路径path:$path');
    try {
      db = await openDatabase(path, version: version);
      _dbCount = 1;
      mypdebug('DB open $_dbCount');
    } catch (e) {
      mypdebug('DBUtil open() Error $e');
    }
  }

  // 记得及时关闭数据库，防止内存泄漏
  close() async {
    mypdebug('DB close $_dbCount');
    _dbCount--;
    if (_dbCount > 0) {
      return;
    }
