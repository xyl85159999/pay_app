...
import 'package:flutter_tron_api/tron/entity/tron/wallet_entity.dart';
...
class TronWallet {
  ...
  //
  //   KeyData data = ED25519_HD_KEY.derivePath("m/44'/195'", hexSeed);
  //   var pb = ED25519_HD_KEY.getPublicKey(Uint8List.fromList(data.key));
  //
  //   String privateKey = HEX.encode(data.key);
  //
  //   Uint8List key = privateKeyToPublic(hexToInt(privateKey));
  ...
  String getBase58Address(Uint8List address) {
    ...
    Uint8List one = Uint8List.fromList(sha256.convert(result).bytes);
